...
public class Damage : MonoBehaviour {
	...
	private bool isActuallyKilled = false;
	...
	void OnTriggerEnter(Collider collision)						// used for things like bullets, which are triggers.  
	{
		...
		if (damageOnTrigger) {
			...
			if (collision.gameObject.GetComponent<Health>() != null || collision.gameObject.tag == "Rocket") {	// if the hit object has the Health script on it, deal damage
				collision.gameObject.GetComponent<Health>().ApplyDamage(damageAmount);
				print("Collider has health script");
				if(this.gameObject.tag == "Finish")
                {
					print("this is the finish");
					collision.gameObject.GetComponent<Health>().LevelToLoad = "Level2";
					
                } else if (this.gameObject.tag == "Finish2")
                {
					collision.gameObject.GetComponent<Health>().LevelToLoad = "Level1";
				}
